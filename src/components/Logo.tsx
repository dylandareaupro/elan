/* Brand logo mark — the Élan "e" squircle (matches the PWA icon & elan-icon.svg). */
...
export function LogoMark({ P, size = 96, radius = 28, glow = true }: { P: Palette; size?: number; radius?: number; glow?: boolean }) {
  const gid = React.useMemo(() => `elan-grad-${_uid++}`, []);
  return (
    <div style={{
      width: size, height: size, borderRadius: radius, position: "relative", overflow: "hidden", flexShrink: 0,
      boxShadow: glow
        ? `0 18px 48px ${P.primaryDeep}55, inset 0 1px 0 rgba(255,255,255,0.25)`
        : "inset 0 1px 0 rgba(255,255,255,0.25)",
    }}>
      <svg width={size} height={size} viewBox="0 0 100 100" aria-hidden="true" style={{ display: "block" }}>
        {/* indigo squircle — full bleed; corners come from the wrapper radius */}
        <rect width="100" height="100" fill={`url(#${gid})`} />
        {/* subtle top-left sheen */}
        <rect width="100" height="100" fill="url(#elan-sheen)" />
        {/* orange spark (the accent aigu of "Élan") */}
        <path d="M58.3855 26.8346C58.0645 26.6178 58.1014 26.1341 58.4515 25.9684L68.832 21.0548C69.2029 20.8793 69.6156 21.2005 69.5365 21.6032L67.4674 32.1319C67.3984 32.4832 66.9935 32.6502 66.6969 32.4498L58.3855 26.8346Z" fill={P.action} />
        {/* white "e" */}
        ...
        <defs>
          <linearGradient id={gid} x1="0" y1="0" x2="100" y2="100" gradientUnits="userSpaceOnUse">
            <stop stopColor={P.primary} />
            <stop offset="1" stopColor={P.primaryDeep} />
          </linearGradient>
          <linearGradient id="elan-sheen" x1="0" y1="0" x2="48" y2="48" gradientUnits="userSpaceOnUse">
